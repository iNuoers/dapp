...
export var AAVEGatewayEvents;
(function (AAVEGatewayEvents) {
    AAVEGatewayEvents["OwnershipTransferred"] = "OwnershipTransferred";
})(AAVEGatewayEvents || (AAVEGatewayEvents = {}));
export class AAVEGatewayContract extends BaseContract {
    constructor(address, supportedProvider, ww3) {
        super('AAVEGateway', AAVEGatewayContract.ABI(), address, supportedProvider, ww3);
        this._methodABIIndex = {};
        this._subscriptionManager = new SubscriptionManager(AAVEGatewayContract.ABI(), supportedProvider);
    ...
    static async init(contract_address, supportedProvider, ww3) {
        const contractInstance = await new AAVEGatewayContract(contract_address, supportedProvider, ww3);
    ...
    async approveAsset(asset) {
        ...
        const promizz = self._contract.methods.approveAsset(asset);
        const result = await promizz.send({
            from: this._blockwrap.getAccountAddress(),
            gas: this.gas,
            gasPrice: this.gasPrice
    ...
    ;
    async approveAssetGas(asset) {
        const self = this;
        const gasAmount = await self._contract.methods.approveAsset(asset).estimateGas();
        return gasAmount;
    }
    ;
    async depositAToken(asset, amount) {
        const self = this;
        assert.isString('asset', asset);
        assert.isNumberOrBigNumber('amount', amount);
        const promizz = self._contract.methods.depositAToken(asset, amount);
        const result = await promizz.send({
            from: this._blockwrap.getAccountAddress(),
            gas: this.gas,
            gasPrice: this.gasPrice
    ...
    ;
    async depositATokenGas(asset, amount) {
        const self = this;
    ...
    subscribe(eventName, indexFilterValues, callback, isVerbose = false, blockPollingIntervalMs) {
        ...
        const subscriptionToken = this._subscriptionManager.subscribe(this._address, eventName, indexFilterValues, AAVEGatewayContract.ABI(), callback, isVerbose, blockPollingIntervalMs);
    ...
    async getLogsAsync(eventName, blockRange, indexFilterValues) {
        ...
        assert.doesConformToSchema('indexFilterValues', indexFilterValues, schemas.indexFilterValuesSchema);
        const logs = await this._subscriptionManager.getLogsAsync(this._address, eventName, blockRange, indexFilterValues, AAVEGatewayContract.ABI());
        return logs;
    }
}
AAVEGatewayContract.contractName = 'AAVEGateway';

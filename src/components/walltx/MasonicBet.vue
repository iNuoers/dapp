...
export default {
  ...
  computed: {
    _disabled () {
      const result = !this.$store.getters["masonic/isNewAccount"]
      if (result) {
  ...
  watch: {
    ...
    local_upline_code (o, n) {
      if (o.length === 6) {
        ...
        this.$store.dispatch("trc913/enter_upline", o)
      } else if (!this.$store.getters["trc913/registered_user"]) {
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      this.status_ms = 4
      if (!this.$store.getters["trc913/registered_user"]) {
        this.invite_code = this.$store.getters["trc913/upline"]

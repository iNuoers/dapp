(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.svgVdom = {}));
	...
	})(exports.selectorUnitCombinator || (exports.selectorUnitCombinator = {}));
	...
	})(exports.attrModifier || (exports.attrModifier = {}));
	// 用于验证的正则表达式
	// css 选择器相关字符
	const idChar = '#[^#\\.\\[\\*:\\s,]+';
	const classChar = '\\.[^#\\.\\[\\*:\\s,]+';
	...
	const parseSelectorUnit = (selector) => {
	    ...
	    while (selectorExec && selectorExec[0].length) {
	                ...
	                switch (specialExec[0][0]) {
	                    ...
	                    case '[': { // 属性选择器
	                        ...
	                        if (eqIndex === -1) {
	                            ...
	                            // 取出等号修饰符
	                            if (typeof exports.attrModifier[attrStr[eqIndex - 1]] === 'number') {
	                                selectorUnit.attr.push({
	                                    key: attrStr.slice(0, eqIndex - 1),
	                                    modifier: exports.attrModifier[attrStr[eqIndex - 1]],
	        ...
	        if (selectorExec[3]) {
	            ...
	            if (typeof exports.selectorUnitCombinator[combinator] === 'number') {
	                selectorUnit.combinator = exports.selectorUnitCombinator[combinator];
	            }
	        }
	...
	const parseSelector = (query) => {
	    ...
	    while (queryExec) {
	        ...
	        if (queryExec[1]) {
	            selectors.unshift({
	                combinator: exports.selectorUnitCombinator['&>'],
	                id: [],
	                class: [],
	                attr: [],
	                pseudo: [],
	...
	const checkClass = (node, selector) => {
	    if (node.nodeType !== exports.NodeType.Tag)
	        return false;
	        ...
	        if (validPseudoElement.includes(pseudoSelector.func)) {
	            ...
	            switch (pseudoSelector.func) {
	                ...
	                case 'focus': // CSS2 伪类，可命中任意元素
	                    return true;
	                case 'root':
	                    return !node.parentNode || node.parentNode.nodeType !== exports.NodeType.Tag;
	...
	const matchSelectors = (selectors, node, finder) => {
	    ...
	    while (currentSelectorIndex >= 0) {
	        ...
	        switch (selectors[currentSelectorIndex].combinator) {
	            // finder 的子元素
	            case exports.selectorUnitCombinator['&>']:
	            ...
	            case exports.selectorUnitCombinator['+']:
	                if (currentNode.parentNode) {
	                    ...
	                    let brother;
	                    while (nodeIndex > 0) {
	                        if (brothers[nodeIndex - 1].nodeType === exports.NodeType.Tag) {
	            ...
	            case exports.selectorUnitCombinator['~']:
	                if (currentNode.parentNode) {
	                    ...
	                    let _brother;
	                    for (let bi = index; bi--;) {
	                        if (_brothers[bi].nodeType === exports.NodeType.Tag) {
	...
	const traversal = (condition, node, breakImmediate, result) => {
	    ...
	    if (node.nodeType === exports.NodeType.Tag || node.nodeType === exports.NodeType.Document) {
	...
	class ParentNode extends Node {
	    ...
	    get children() {
	        ...
	        for (const child of this.childNodes) {
	            if (child.nodeType === exports.NodeType.Tag) {
	                children.push(child);
	            }
	        }
	        return children;
	    ...
	    appendChild(childNode) {
	        ...
	        if (childNode.nodeType === exports.NodeType.DocumentFragment) {
	    ...
	    replaceChild(newChild, oldChild) {
	        ...
	        if (Array.isArray(newChild)) {
	            ...
	            [...newChild].forEach(child => {
	                ...
	                if (child.nodeType === exports.NodeType.DocumentFragment) {
	        ...
	        // DocumentFragment 需要特殊处理
	        if (newChild.nodeType === exports.NodeType.DocumentFragment) {
	...
	class TagNode extends ParentNode {
	    constructor(option) {
	        super(option);
	        this.nodeType = exports.NodeType.Tag;
	        this.attributes = [];
	    }
	    /**
	     * 复制自身，但是不复制节点树关系链
	...
	const configs = [
	    ['xml-decl', REG_XML_DECL, exports.NodeType.XMLDecl],
	    ['cdata', REG_CDATA_SECT, exports.NodeType.CDATA],
	    ['doctype', REG_DOCTYPE, exports.NodeType.DocType],
	    ['comments', REG_COMMENTS, exports.NodeType.Comments],
	];
	...
	const parse = async (str) => {
	    return new Promise((resolve, reject) => {
	        ...
	        if (current.node.nodeType === exports.NodeType.CDATA) {
	        ...
	        while (current.lastIndex < len) {
	            ...
	            if (current.node.nodeType === -1) {
	                ...
	                else if (current.node.nodeType === exports.NodeType.CDATA) {
	                    // CDATA 节点禁止出现在根下
	                    reject(new Error(`Invalid CDATA element! At ${status.line}:${status.pos}`));
	                    return;
	                }
	                else if (current.node.nodeType === exports.NodeType.Text) {
	                    // 没有节点而出现了非空文本节点
	...
	exports.matchSelectorGroups = matchSelectorGroups;

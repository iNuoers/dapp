...
function Square(kx, ky) {
  ...
  // `hsl(${intAlea(360)},100%,50%)`;
  // color ? color : `hsl(${intAlea(360)},100%,50%)`;
  this.kx = kx
  this.ky = ky
  this.kxc = kx - hnbx
  this.kyc = ky - hnby
...
Square.prototype.lineFromTo = function (sd1, sd2) {
  ...
  if (this.kxc === this.kyc) { // symm around 1st bisector
    if (sd2 === 1) { drawQuarter(this, 3) } else if (sd2 === 3) { drawQuarter(this, 1) }
  }
  if (this.kxc === -this.kyc) { // symm around 1st bisector
    if (sd2 === 0) { drawQuarter(this, 2) } else if (sd2 === 2) { drawQuarter(this, 0) }
  }
...
function generateBackgroundCanvas(elementID, click_gen, auto_gen_sec, c1, c2, c3,
  ...
  if (click_gen) {
    ...
    // just for initial position
  } else if (auto_gen_sec >= 1000) {
  ...
  if (element === undefined) {
    ...
    generation_mode = 1
    maxx = element.parentElement.clientWidth
    maxy = element.parentElement.clientHeight
  }

  if (useCustomColor) {
    const select_color = intAlea(1, 3)
    let C
    switch (select_color) {
      case 1:
        C = hexToRgb(c1)
    ...
    const cd = new ColorTool(C.r, C.g, C.b, "love")
    customhue = cd.data().h
    customsq = `hsl(${customhue},50%,40%)`
  } else {
    customhue = intAlea(360)
    customsq = `hsl(${customhue},90%,60%)`
  }
  events = [{ event: "reset" }]

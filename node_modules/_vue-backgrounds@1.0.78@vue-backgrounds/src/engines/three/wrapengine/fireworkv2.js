...
"use strict"
import { MyMath, Stage } from "./mymath"
import { HALF_PI, PI_2 } from "./mathlib"

console.clear()
let IS_MOBILE = window.innerWidth <= 640
let IS_DESKTOP = window.innerWidth > 800
let IS_HEADER = IS_DESKTOP && window.innerHeight < 300

function update_screen_settings() {
...
function toggleFullscreen() {
  if (fullscreenEnabled()) {
    if (isFullscreen()) {
      ...
      document.requestFullscreen(document.documentElement)
    }
  }
}
...
function init() {
  const canvas_trails = document.getElementById("trails-canvas")
  const canvas_main = document.getElementById("main-canvas")
  const trailsStage = new Stage(canvas_trails)
  const mainStage = new Stage(canvas_main)
...
function createBurst(count, particleFactory, startAngle = 0, arcLength = PI_2) {
  ...
  for (let i = 0; i <= C_HALF; i++) {
    const ringAngle = i / C_HALF * HALF_PI
...
function crossetteEffect(star) {
  const startAngle = Math.random() * HALF_PI
...
class Shell {
  ...
  burst(x, y) {
    ...
    if (typeof this.color === "string") {
      ...
      throw new TypeError("Invalid shell color. Expected string or array of strings, but got: " + this.color)
    }
...
const soundManager = {
  ...
  playSound(type, scale = 1) {
    ...
    const initialPlaybackRate = MyMath.random(source.playbackRateMin,
